...
function exportCSV(data: Encaissement[]) {
  const headers = ["Date", "Référence", "Catégorie", "Description", "Client", "Chambre", "Montant", "Mode"];
  ...
  a.href = url;
  a.download = `encaissements-${getMonthKey()}.csv`;
  a.click();
...
export default function RapportEncaissement() {
  ...
  const [editForm, setEditForm] = useState<Partial<Encaissement>>({});
  ...
  const handleDelete = async (id: number) => {
    try {
      const res = await fetch(`/api/encaissements/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const startEdit = (row: Encaissement) => {
    setEditingId(row.id);
    setEditForm({
      date: row.date,
  ...
  const handleSaveEdit = async () => {
    if (!editingId) return;
    try {
      const res = await fetch(`/api/encaissements/${editingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(editForm),
      });
      if (res.ok) {
        toast("Encaissement modifié");
        setEditingId(null);
        fetchData();
      } else {
    ...
    <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="mx-4 w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-brown">
              &Ecirc;tes-vous s&ucirc;r de vouloir supprimer cet encaissement ? Cette action est irr&eacute;versible.
            </p>
      ...
      {editingId !== null && (
        <div className="rounded-xl border border-gold bg-amber-light p-4">
          <h3 className="mb-3 text-sm font-semibold text-brown-dark">Modifier l&apos;encaissement</h3>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-brown">Client</label>
              <input
                type="text"
          ...
          <table className="w-full min-w-[900px] text-left text-sm">
            <thead>
              <tr className="border-b border-cream-dark">
                ...
                <th className="pb-3 pr-3 text-xs font-semibold uppercase tracking-wide text-brown">Client</th>
                <th className="pb-3 pr-3 text-xs font-semibold uppercase tracking-wide text-brown">Chambre</th>
            ...
            <tbody>
              {data.map((row) => (
                <tr key={row.id} className="border-b border-cream-dark last:border-0">
                  ...
                  <td className="py-3 pr-3 whitespace-nowrap text-brown">{row.client || "\u2014"}</td>
                  <td className="py-3 pr-3 whitespace-nowrap text-brown">{row.roomName || "\u2014"}</td>
                  <td className="py-3 pr-3 whitespace-nowrap text-right font-semibold text-green">{formatMAD(parseFloat(row.amount))}</td>
